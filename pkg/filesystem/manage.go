...
import (
	...
	"github.com/songquanpeng/one-api/pkg/filesystem/fsctx"
...
func (fs *FileSystem) Rename(ctx context.Context, dir, file []uint, new string) (err error) {
	...
	// 如果源对象是文件
	if len(file) > 0 {
		fileObject, err := model.GetFilesByIDs([]uint{file[0]}, uint(fs.User.Id))
	...
	if len(dir) > 0 {
		folderObject, err := model.GetFoldersByIDs([]uint{dir[0]}, uint(fs.User.Id))
...
func (fs *FileSystem) Delete(ctx context.Context, dirs, files []uint, force, unlink bool) error {
	...
	// 删除文件记录
	err = model.DeleteFiles(deletedFiles, uint(fs.User.Id))
	if err != nil {
...
func (fs *FileSystem) ListDeleteDirs(ctx context.Context, ids []uint) error {
	// 列出所有递归子目录
	folders, err := model.GetRecursiveChildFolder(ids, uint(fs.User.Id), true)
	if err != nil {
		return ErrDBListObjects.WithError(err)
	}
...
func (fs *FileSystem) ListDeleteFiles(ctx context.Context, ids []uint) error {
	files, err := model.GetFilesByIDs(ids, uint(fs.User.Id))
	if err != nil {
		return ErrDBListObjects.WithError(err)
	}
	fs.SetTargetFile(&files)
