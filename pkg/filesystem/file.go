...
import (
	...
	"github.com/songquanpeng/one-api/pkg/filesystem/response"
...
func (fs *FileSystem) resetFileIDIfNotExist(ctx context.Context, id uint) error {
	// 找到文件
	if len(fs.FileTarget) == 0 {
		file, err := model.GetFilesByIDs([]uint{id}, uint(fs.User.Id))
...
func (fs *FileSystem) Search(ctx context.Context, keywords ...interface{}) ([]serializer.Object, error) {
	...
	if fs.Root != nil {
		allFolders, err := model.GetRecursiveChildFolder([]uint{fs.Root.ID}, uint(fs.User.Id), true)
		if err != nil {
	...
	files, _ := model.GetFilesByKeywords(uint(fs.User.Id), parents, keywords...)
